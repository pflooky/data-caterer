...
class ValidationConfigurationBuilderTest extends AnyFunSuite {
  ...
  test("Can create simple validation for data source") {
    ...
    assert(result.dataSources.head._2.validations.contains(ExpressionValidation("amount < 100")))
    assert(result.dataSources.head._2.validations.contains(ExpressionValidation("name == 'Peter'")))
  }
